...
(function() {
    ...
    f.masterSkillCoolDown = function(player, sid) {
        ...
        if (meta == null) {
            console.log("yby server error !!!sid" + sid);
            return;
        }        
    ...
    f.petExpLevelRank = [1, 20, 60, 80, 120, 140, 180];
    f.petExpCoe = [100, 500, 1500, 5000, 12000, 30000, 80000];

    /**
     * 根据当前等级，获取宠物该等级升级所需的最大经验值
     * @param petLevel          宠物等级
     * @returns {number}        该等级升级所需的最大经验值
     */
    f.getMaxPetExp = function(petLevel){
        var base = 0;
        var len = f.petExpLevelRank.length;

        for (var i = 0, j = len; i < j; ++ i){
            if (f.petExpLevelRank[i] > petLevel) {
                base += (f.petExpCoe[i - 1] * (petLevel - f.petExpLevelRank[i - 1]));
                break;
            }
            if (i == 0) base += f.petExpCoe[i];
            else base += (f.petExpCoe[i - 1] * (f.petExpLevelRank[i] - f.petExpLevelRank[i - 1]));
        }
        if (petLevel > f.petExpLevelRank[len - 1]){
            base += (f.petExpCoe[len - 1] * (petLevel - f.petExpLevelRank[len - 1]));
        }
        return base;
    };

    f.petLevelRank = [1, 50, 100, 150, 200];
    f.petBaseHP = [320, 1888, 5088, 11488, 24288];
    f.petBaseAttack = [40, 235, 636, 1436, 3036];
    f.petRankRate = [40, 60, 80, 100, 150, 225, 338, 506, 759, 1139, 1709, 2563, 3844, 5767, 8650, 12975, 19462, 29193, 43789, 65684, 98526, 147789];

    /**
     * 获取指定宠物等级，指定宠物品阶的宠物血量以及对应的攻击力
     * @param petLevel  宠物等级
     * @param petRank   宠物品阶
     * @returns {{bHp: number, bAtt: number}}
     */
    f.getPetHpAndAttack = function(petLevel, petRank){
        var rate = f.petRankRate[petRank - 1];
        var idx = 0;
        for (var i = 0, j = f.petLevelRank.length; i < j; ++ i){
            if (f.petLevelRank[i] > petLevel) break;
            idx = i;
        }
        return {
            hp : Math.round((f.petBaseHP[idx] + (((idx + 1) * f.petBaseHP[0] / 10) * (petLevel - f.petLevelRank[idx]))) * rate / 100),
            attack : Math.round((f.petBaseAttack[idx] + (((idx + 1) * f.petBaseAttack[0] / 10) * (petLevel - f.petLevelRank[idx]))) * rate / 100)
        };
    };
     ...
     */
    f.isCrit = function(){
        return Util.randomRate(0.2);
    };

    f.scoreStep = [999, 1499, 1999, 2499, 2999, 3499, 9999999];

    /**
     * 获取最大分数
     */
    f.getMaxScore = function(){
        return f.scoreStep[f.scoreStep.length -1];
    };
    /**
     * 通过分数来获取阶级
     * @param score
     */
    f.getStep = function(score){
        var step = 0;
        for (var i = 0, j = f.scoreStep.length; i < j; ++i){
            if (f.scoreStep[i] > score){
                break;
            }
            step = i;
        }
     ...
     * @returns {number}
     */
    f.getModScore = function(score, ownerRank, enemyRank){
        return score + f.mScores[f.getStep(score)] + ownerRank - enemyRank;
    };
